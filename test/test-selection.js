...
describe("Selection", () => {
  ...
  it("moves after an inserted leaf node", () => {
    ...
    sameDoc(state.doc, doc(p("foo"), hr, p("bar")), "split paragraph")
    assert.equal(state.selection.head, 7, "moved after rule")
    state.textSel(10)
    state.apply(state.tr.replaceSelection(schema.node("horizontal_rule")))
    sameDoc(state.doc, doc(p("foo"), hr, p("bar"), hr), "inserted after")
  ...
  it("allows typing over a leaf node", () => {
    ...
    sameDoc(state.doc, doc(p("a"), p("x"), p("b")))
    assert.equal(state.selection.head, 5)
    assert.equal(state.selection.anchor, 5)
  })
  ...
  it("allows deleting a selected block", () => {
    ...
    sameDoc(state.doc, doc(ul(li(p("bar")), li(p("baz")), li(p("quux")))), "paragraph vanished")
    assert.equal(state.selection.head, 3, "moved to list")
    state.nodeSel(2)
    state.deleteSelection()
    sameDoc(state.doc, doc(ul(li(p("baz")), li(p("quux")))), "delete whole item")
    assert.equal(state.selection.head, 3, "to next item")
    state.nodeSel(9)
    state.deleteSelection()
    sameDoc(state.doc, doc(ul(li(p("baz")))), "delete last item")
    assert.equal(state.selection.head, 6, "back to paragraph above")
    state.nodeSel(0)
    state.deleteSelection()
    sameDoc(state.doc, doc(p()), "delete list")
  })
  ...
  it("allows deleting a leaf", () => {
    ...
    state.nodeSel(3)
    state.deleteSelection()
    sameDoc(state.doc, doc(p("a"), hr, p("b")), "deleted first hr")
    assert.equal(state.selection.from, 3, "moved to second hr")
    state.deleteSelection()
    sameDoc(state.doc, doc(p("a"), p("b")), "deleted second hr")
  ...
  it("properly handles deleting the selection", () => {
    ...
    sameDoc(state.doc, doc(p("foobar"), blockquote(p("hi")), p("ay")), "deleted img")
    assert.equal(state.selection.head, 4, "cursor at img")
    state.nodeSel(9)
    state.apply(state.tr.deleteSelection())
    sameDoc(state.doc, doc(p("foobar"), p("ay")), "deleted blockquote")
    assert.equal(state.selection.from, 9, "cursor moved past")
    state.nodeSel(8)
    state.apply(state.tr.deleteSelection())
    sameDoc(state.doc, doc(p("foobar")), "deleted paragraph")
  ...
  it("can replace inline selections", () => {
    ...
    sameDoc(state.doc, doc(p("foo", br, "bar", img, "baz")), "replaced with br")
    ...
    sameDoc(state.doc, doc(p("foo", br, "barabcbaz")), "replaced with text")
    ...
    sameDoc(state.doc, doc(p("xyz")), "replaced all of paragraph")
  })
  ...
  it("can replace a block selection", () => {
    ...
    sameDoc(state.doc, doc(p("abc"), pre(), hr, blockquote(p("ow"))), "replace with code block")
    assert.equal(state.selection.from, 7, "selection after")
    state.nodeSel(8)
    state.apply(state.tr.replaceSelection(schema.node("paragraph")))
    sameDoc(state.doc, doc(p("abc"), pre(), hr, p()), "replace with paragraph")
