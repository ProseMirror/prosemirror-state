import { Slice, Fragment, ResolvedPos, Schema, Node as ProsemirrorNode } from "prosemirror-model";
...
export class Selection<S extends Schema = any> {
  ...
  replaceWith(tr: Transaction<S>, node: ProsemirrorNode<S>) {
    let mapFrom = tr.steps.length,
      ranges = this.ranges;
    for (let i = 0; i < ranges.length; i++) {
      let { $from, $to } = ranges[i],
  ...
  // found.
  static findFrom<S extends Schema = any>($pos: ResolvedPos<S>, dir: number, textOnly?: boolean) {
  ...
  static near<S extends Schema = any>($pos: ResolvedPos<S>, bias = 1) {
  ...
  // exists.
  static atStart<S extends Schema = any>(doc: ProsemirrorNode<S>) {
  ...
  // given document.
  static atEnd<S extends Schema = any>(doc: ProsemirrorNode<S>) {
  ...
  static jsonID(id: string, selectionClass: { new (...args: any[]): Selection }) {
...
export class TextSelection<S extends Schema = any> extends Selection {
  ...
  static between<S extends Schema = any>($anchor: ResolvedPos<S>, $head: ResolvedPos<S>, bias?: 1 | -1) {
...
export class NodeSelection<S extends Schema = any> extends Selection {
  $pos: ResolvedPos<S>;
...
// text selections.
function findSelectionIn<S extends Schema = any>(
...
function selectionToInsertionEnd<S extends Schema = any>(tr: Transaction<S>, startLen: number, bias: 1 | -1) {
